...
#define ACC_WEIGHT_MAX 0.01		//maximum accelerometer weight in accelerometer-gyro fusion formula
...
void imu_update(float interval_ms){
	int i;
	imu_sequence++;
...
*/
    ...
    // printf(" interval_ms %1.3f ",interval_ms);
	for(i=0;i<3;i++){
		w[i] = w[i]*interval_ms;				//scale by elapsed time to get angle in radians
